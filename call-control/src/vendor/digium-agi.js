...
function createSimpleMethod(command) {
    ...
    command.toLowerCase().split(' ').forEach(function (word, idx) {
...
Context.prototype.sayNumber = function (number, escape_digits) {
    debug('sayNumber', number, escape_digits);
    return this.request('SAY NUMBER ' + number + ' "' + escape_digits + '"' + '\n');
};

Context.prototype.sayAlpha = function (number, escape_digits) {
    debug('sayAlpha', number, escape_digits);
    return this.request('SAY ALPHA ' + number + ' "' + escape_digits + '"' + '\n');
};

Context.prototype.sayDate = function (seconds, escape_digits) {   //seconds since 1.01.1970
    debug('sayDate', seconds, escape_digits);
    return this.request('SAY DATE ' + seconds + ' "' + escape_digits + '"' + '\n');
};

Context.prototype.sayTime = function (seconds, escape_digits) {   //seconds since 1.01.1970
    debug('sayTime', seconds, escape_digits);
    return this.request('SAY TIME ' + seconds + ' "' + escape_digits + '"' + '\n');
};

Context.prototype.sayDateTime = function (seconds, escape_digits, format, timezone) {   //seconds since 1.01.1970
    debug('sayDateTime', seconds, escape_digits, format, timezone);
    return this.request('SAY DATETIME ' + seconds + ' "' + escape_digits + '" ' + format + ' ' + timezone + '\n');
};

Context.prototype.sayDigits = function (digits, escape_digits) {
    debug('sayDigits', digits, escape_digits);
    return this.request('SAY DIGITS ' + digits + ' "' + escape_digits + '"' + '\n');
};

Context.prototype.sayPhonetic = function (string, escape_digits) {
    debug('sayPhonetic', string, escape_digits);
    return this.request('SAY PHONETIC ' + string + ' "' + escape_digits + '"' + '\n');
};
